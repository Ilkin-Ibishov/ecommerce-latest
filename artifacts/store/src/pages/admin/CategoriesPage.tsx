...
const EMPTY_FORM = { slug: "", icon_url: "", translations: LANGS.map((lang_code) => ({ lang_code, title: "" })) };
...
export default function AdminCategoriesPage() {
  ...
  useEffect(() => {
    ...
    (supabase as any).from("categories").select("*, category_translations(*)").is("parent_id", null).order("id")
  ...
  const openEdit = (c: Category) => {
    setEditing(c);
    setForm({ slug: c.slug, icon_url: c.icon_url ?? "", translations: LANGS.map((lang_code) => ({ lang_code, title: c.category_translations.find((t) => t.lang_code === lang_code)?.title ?? "" })) });
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <F label="Icon URL" value={form.icon_url} onChange={(v) => setForm((f) => ({ ...f, icon_url: v }))} placeholder="https://..." />
          </div>
          <div className="space-y-3">
            {LANGS.map((lang) => (
              <F key={lang} label={`Title (${lang})`} value={form.translations.find((t) => t.lang_code === lang)?.title ?? ""} onChange={(v) => setTitle(lang, v)} placeholder={`Category name in ${lang}`} />
            ))}
          </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {categories.length === 0 ? (
              ...
              const title = c.category_translations.find((t) => t.lang_code === "az")?.title ?? c.slug;
              return (
                <tr key={c.id} className="border-b border-border/50 hover:bg-muted/20">
                  <td className="px-4 py-3 font-medium flex items-center gap-2">
                    {c.icon_url && <img src={c.icon_url} alt="" className="w-6 h-6 rounded-full object-cover" />}
                    {title}
                  </td>
