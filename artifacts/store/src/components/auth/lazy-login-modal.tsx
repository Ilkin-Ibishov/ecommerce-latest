  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        } else if (data.reason === "rate_limit_hour") {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.reason === "invalid_code") {
          ...
          setStep("phone");
        } else if (data.reason === "not_found_or_expired") {
      ...
      // Sign in with the token from verify endpoint
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-background rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 border border-border">
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-accent transition"
        >
          <X size={18} />
        </button>
            ...
            <p className="text-sm text-muted-foreground mb-6">
              ...
              {process.env.NODE_ENV === "development" && (
                <span className="block mt-1 text-orange-600 text-xs">
                  DEV MODE: Check server console for the OTP code.
                </span>
              )}
            </p>
            <form onSubmit={handleOtpSubmit} className="space-y-4">
              ...
              <div className="text-center">
                {cooldown > 0 ? (
                  ...
                  <button
                    type="button"
                    onClick={handlePhoneSubmit as any}
