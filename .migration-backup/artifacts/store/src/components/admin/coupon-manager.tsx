...
export default function CouponManager({ initialCoupons }: { initialCoupons: Coupon[] }) {
  ...
  const openEdit = (c: Coupon) => {
    ...
    setForm({
      ...
      expires_at: c.expires_at ? c.expires_at.split("T")[0] : null,
    });
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      if (data.id) {
        setCoupons((prev) => [{ id: data.id, used_count: 0, ...form } as Coupon, ...prev]);
      }
    }
    setSaving(false);
    setShowForm(false);
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Code" value={form.code} onChange={(v) => setForm((f) => ({ ...f, code: v.toUpperCase() }))} placeholder="SUMMER20" required />
            <Field label="Description" value={form.description ?? ""} onChange={(v) => setForm((f) => ({ ...f, description: v }))} placeholder="Optional note" />
            <div>
            ...
            <NumberField
              label={form.discount_type === "percentage" ? "Discount %" : "Discount AZN"}
              value={form.discount_value}
              onChange={(v) => setForm((f) => ({ ...f, discount_value: v }))}
            />
            <NumberField label="Min Order (AZN)" value={form.min_order_amount ?? 0} onChange={(v) => setForm((f) => ({ ...f, min_order_amount: v || null }))} />
            <NumberField label="Max Uses (0 = unlimited)" value={form.max_uses ?? 0} onChange={(v) => setForm((f) => ({ ...f, max_uses: v || null }))} />
            <div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {coupons.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-10 text-center text-muted-foreground">No coupons yet.</td>
              </tr>
            ) : (
              coupons.map((c) => (
                <tr key={c.id} className="border-b border-border/50 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => toggleActive(c)} className={`px-2 py-0.5 rounded-full text-xs font-medium transition ${c.is_active ? "bg-green-500/20 text-green-400" : "bg-muted text-muted-foreground"}`}>
